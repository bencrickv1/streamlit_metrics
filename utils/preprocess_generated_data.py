...
def preprocess_generated_data(input_json, output_json):
    ...
    data_df['submission_date'] = pd.to_datetime(data_df['submission_date'], format='ISO8601')
    data_df['application_type'] = data_df.apply(lambda row: row['application_type'].replace('Planning permission', 'Planning Permission'), axis=1)
    ...
    for application_type in set(data_df['application_type']):
        for application_category in application_categories:
            if application_type.strip()[:len(application_category)]==application_category:
                application_category_map[application_type] = application_category

    data_df['application_category'] = data_df.apply(lambda row: application_category_map[row['application_type']], axis=1)
    ...
    display_gdf.sort_values(
        by=['application_category', 'application_type'],
        ascending=True,
        inplace=True
    )

    display_gdf['readable_fee'] = display_gdf.apply(lambda row: numerize(row['fee'], 2), axis=1)
...
def create_area_data(area_gdf, preprocessed_json, output_json):
    ...
    area_gdf['applications_per_capita'] = area_gdf['number_of_applications'] / area_gdf['LPA_population']
    area_gdf['applications_per_km2'] = area_gdf['number_of_applications'] / area_gdf['LPA_area_km2']

    area_gdf['total_fee_per_capita'] = area_gdf['total_fee'] / area_gdf['LPA_population']
